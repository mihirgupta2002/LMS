...
export default function Mainpage() {
  ...
  const{id}=useParams();
    ...
    const{BOOKInfo,MEMBERInfo,bookRequestList,addRequest,AUTHORInfo}=useContext(GlobalContext)
  ...
  Login/Signup .
     ...
     <div className="library-card-container" style={{display:"flex", justifyContent:"flex-start",direction:"column"}}>
      ...
      {BOOKInfo.map((book,index)=>(
        ...
        <LibraryCard quantity={book.quantity}
        
        
        
        image={book.image} bookIssue={()=>{  setShowAlert(true);
        setTimeout(()=> setShowAlert(false),3000);}} selectedClick={()=>{setShowAlert(true);
        setTimeout(()=> setShowAlert(false),3000);}} title={book.title} authorName={book.authorName} description={book.description} isbn={book.isbn} />
      ))}
      </div>
   </div>
